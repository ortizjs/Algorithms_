def merge_meeting_times(meeting_times)
    ...
    (1...sorted_meeting_times.length).each do |i|
        current_meeting_time = sorted_meeting_times[i]
        last_merged_meeting = merged_meeting_times[merged_meeting_times.length - 1]
        if current_meeting_time[:startTime] <= last_merged_meeting[:endTime]
            last_merged_meeting[:endTime] = [last_merged_meeting[:endTime], current_meeting_time[:endTime]].max
        else
